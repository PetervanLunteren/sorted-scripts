...
# # Function to display images in a grid with user input to view next batch
# def show_images_interactive(data_loader, num_images=BATCH_SIZE):
...
#             img = img.permute(1, 2, 0)  # Change dimensions to HWC for visualization
#             img = img * torch.tensor([0.229, 0.224, 0.225]) + torch.tensor([0.485, 0.456, 0.406])  # Denormalize
#             img = torch.clip(img, 0, 1)  # Clip values to valid range
#             axes[i].imshow(img)
...
#         # Wait for user input to proceed to the next batch
#         input("Press Enter to view the next batch...")

# # Load and display images in batches interactively
# show_images_interactive(train_loader)
...
# import timm  # For loading the backbone
# import os

# # # user input
# # dst_dir = r"C:\Users\smart\Desktop\test_df"

# # set training dir
# current_train_set = r"C:\Peter\training-utils\current-train-set"
# os.chdir(current_train_set)
...
# # Configuration
# CROP_SIZE = 224
# # BACKBONE = "convnext_base"  # Updated backbone # TEST
# BACKBONE = "vit_large_patch14_dinov2.lvd142m"
# NUM_CLASSES = len(next(os.walk(train_dir_path))[1])  # Counts subfolders
# EPOCHS = 100
# BATCH_SIZE = 16
# LEARNING_RATE = 4e-4
# PATIENCE = 10
...
#     iaa.Affine(rotate=(-10, 10), shear=(-5, 5)),  # Affine transformations
#     iaa.Grayscale(alpha=(0.0, 1.0)),  # Grayscale transformation
#     iaa.Sometimes(0.5, iaa.GaussianBlur(sigma=(0, 1.0))),  # Blurring
...
# train_transforms = transforms.Compose([
#     ImgAugTransform(),  # Apply imgaug augmentations
...
#         patience_counter = 0
#         torch.save({'state_dict': model.state_dict(), 'num_classes': NUM_CLASSES, 'backbone': BACKBONE}, "best_model.pt")
#     else:
#         patience_counter += 1
#         if patience_counter >= PATIENCE:
#             print("Early stopping triggered.")
...
# # Final model saving
# dst_filename = f"final_model_{int(time.time())}.pt"
# torch.save({'state_dict': model.state_dict(), 'num_classes': NUM_CLASSES, 'backbone': BACKBONE}, dst_filename)
...
# CROP_SIZE = 182
# BACKBONE = "vit_large_patch14_dinov2.lvd142m"
# NUM_CLASSES = len(next(os.walk('train'))[1])
# EPOCHS = 20
# BATCH_SIZE = 16
# LEARNING_RATE = 0.001
...
# test_data = datasets.ImageFolder('test', transform=train_transforms)
...
# # Save the model
# torch.save({'state_dict': model.state_dict(), 'num_classes': NUM_CLASSES, 'backbone': BACKBONE}, "trained_model.pt")
